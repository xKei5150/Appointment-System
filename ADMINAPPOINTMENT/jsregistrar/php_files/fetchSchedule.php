...
// Delete past date
$stmt = $conn->prepare("DELETE FROM tblschedule3 WHERE date < :today");
...
$stmt = $conn->prepare("SELECT date, timeslot, slots FROM tblschedule3 WHERE date BETWEEN :startDate AND :endDate ORDER BY id ASC, timeslot");
$stmt->execute(['startDate' => $startDate, 'endDate' => $endDate]);
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);

// If no schedule for the specified date range, generate it
if (empty($results)) {
    $defaultTimeslots = ["8am-9am", "9am-10am", "10am-11am", "11am-12pm", "1pm-2pm", "2pm-3pm"];
    ...
    while ($currentDay <= $endDay) {
        ...
        if ($currentDay->format('N') < 6) {
            foreach ($defaultTimeslots as $timeslot) {
                $stmt = $conn->prepare("INSERT IGNORE INTO tblschedule3 (date, timeslot, slots) VALUES (:date, :timeslot, 10)");
