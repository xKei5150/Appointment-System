...
$stmt = $conn->prepare("SELECT date, timeslot, availability FROM tblschedule WHERE date BETWEEN :startDate AND :endDate ORDER BY id ASC, timeslot");
    ...
    while ($currentDay <= $endDay) {
        ...
        if ($currentDay->format('N') < 7) {
            foreach ($defaultTimeslots as $timeslot) {
                $stmt = $conn->prepare("INSERT IGNORE INTO tblschedule (date, timeslot, availability) VALUES (:date, :timeslot, 1)");
